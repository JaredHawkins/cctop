...
`com.openai.codex` identifies the Codex app for launch. A persisted `com.openai.codex` value does not select focus. Positive live Codex app-server evidence wins over inherited terminal metadata. Without that evidence, direct terminal/editor/multiplexer metadata wins. Otherwise, cctop opens the exact task in the installed Codex app. If that fails, cctop reports an error instead of opening Finder. When no verified, exact, or recognized route exists, an unrecognized captured bundle identifier activates its app before the Finder fallback. Focus inputs do not control lifecycle, archives, Cleanup, Recent, badges, or notifications.
